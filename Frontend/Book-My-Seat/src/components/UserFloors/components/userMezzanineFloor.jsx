...
const UserMezzanineFloor = ({mezzanineFloorData,reserveData,onSeatSelect,selectedSeat, preferredSeatsDataMezzanineFloor}) => {
  ...
  useEffect(() => {
    if (mezzanineFloorData && reserveData && preferredSeatsDataMezzanineFloor) {
        const combinedData = [];
        console.log("HI",mezzanineFloorData)
        ...
        seats.forEach(row => {
            const newRow = row.map(seat => {
                ...
                const preferredSeat = preferredSeatsDataMezzanineFloor.find(preferredSeat => preferredSeat.seat.seatNumber === seat.id);
                if (reservedSeat) {
...
}, [mezzanineFloorData, seats, preferredSeatsDataMezzanineFloor, reserveData]);
  ...
  return(
    ...
    <div className='mezz-seat-map-container'>
      ...
      <div className='mezz-seat-map-layout-row2'>
        <Paper className='mezz-room-4'>{generateRoomSeats(room4, handleSeatClick,selectedSeat)}</Paper>
        <Paper className='mezz-room-5'>{generateRoomSeats(room5, handleSeatClick,selectedSeat)}</Paper>
        <Paper className='mezz-room-6'>{generateRoomSeats(room6, handleSeatClick,selectedSeat)}</Paper>
        <Paper className='mezz-room-7'>{generateRoomSeats(room7, handleSeatClick,selectedSeat)}</Paper>
        <Paper className='mezz-room-8'>{generateRoomSeats(room8, handleSeatClick,selectedSeat)}</Paper>
      </div>
      <div className='mezz-seat-map-layout-row3'>
        ...
        <Paper className='mezz-room-10'>{generateRoomSeats(room10, handleSeatClick,selectedSeat)}</Paper>
      </div>
    </div>
