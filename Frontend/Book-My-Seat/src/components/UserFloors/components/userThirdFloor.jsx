...
import "../../EditSeats/styles/thirdFloor.css"
import Paper from '@mui/material/Paper'

const generateRoomSeats = (room, selectedSeatId, handleSeatClick) => {
        ...
        {col.map((seat, seatIndex) => (
          <div key={seat && seat.id ? seat.id : `empty-${seatIndex}`} className="seat-container">
            {seat ? (
              <div
                key={seat.id}
                className={`seat ${seat.preferred ? "preferred-seat" : ""} ${seat.booking_status ? "reserved-seat" : (selectedSeatId === seat.id ? "selected-seat" : "available-seat")}`}
...
const UserThirdFloor = ({thirdFloorData,selectedSeat,onSeatSelect,preferredSeatsDataThirdFloor}) => {
  ...
  useEffect(() => {
    ...
    if (thirdFloorData && thirdFloorData.length > 0 && preferredSeatsDataThirdFloor && preferredSeatsDataThirdFloor.length > 0) {
        const combinedData = combineData(seats, thirdFloorData, preferredSeatsDataThirdFloor);
        setCombinedSeats(combinedData);
    } else {
        setCombinedSeats(seats);
    }
}, [thirdFloorData, preferredSeatsDataThirdFloor,seats]);

  const combineData = (seats, thirdFloorData, preferredSeatsData) => {
      // Combine seats and thirdFloorData based on seat_number
      const combinedData = seats.map(row => {
          return row.map(seat => {
              if (seat === null) return null;
              const matchingSeat = thirdFloorData.find(fetchedSeat => fetchedSeat.seat_number === seat.id);
              if (matchingSeat) {
                  const preferredSeat = preferredSeatsData.find(preferredSeat => preferredSeat.seat.seatNumber === matchingSeat.seat_number);
                  if (preferredSeat) {
                      return { ...seat, ...matchingSeat, preferred: true };
                  } else {
