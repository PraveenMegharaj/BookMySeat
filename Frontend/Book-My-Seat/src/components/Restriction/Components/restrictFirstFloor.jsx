...
const ReservedFirstFloor = ({firstFloorData, firstFloorReservedData, onSeatClick}) => {
  ...
  useEffect(() => {
    if (firstFloorData && firstFloorReservedData) {
      ...
      seats.forEach(row => {
        const newRow = row.map(seat => {
          ...
          const reservedSeat = firstFloorReservedData.find(reservedSeat => reservedSeat.seat.seatNumber === seat.id);
          if (reservedSeat) {
  ...
  }, [firstFloorData,seats, firstFloorReservedData]);
